...
function exitWorkerGracefully() {
    ...
    Promise.all([db.disconnect(), cluster.worker.disconnect()]).catch(exitWorkerUngracefully);
}
...
if (!cluster.worker) {
    ...
    const key = config.get("FS_SSL_KEY");
    const cert = config.get("FS_SSL_CERT");
    const options = {
        port: SERVICE_PORT,
    ...
    process.on("uncaughtException", (err) => {
        if (err.code === "ERR_IPC_DISCONNECTED")
            return;
        logger.error({
