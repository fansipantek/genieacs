...
function putActionHandler(action, _object, isNew) {
    return new Promise((resolve, reject) => {
        ...
        if (action === "save") {
            ...
            delete object["_id"];
            if (!id)
                return void resolve({ _id: "ID can not be empty" });
            store
                ...
                store
                    ...
                    .then(() => {
                    notifications.push("success", `Virtual parameter ${exists ? "updated" : "created"}`);
                    store.setTimestamp(Date.now());
                    resolve(null);
                })
                    .catch((err) => {
    ...
    return {
        view: (vnode) => {
                ...
                attrs["actionsCallback"] = (selected) => {
                    return [
                        ...
                        m("button.primary", {
                            ...
                            onclick: (e) => {
                                if (!confirm(`Deleting ${selected.size} virtual parameters. Are you sure?`))
                                    return;
                                e.redraw = false;
                                e.target.disabled = true;
                                Promise.all(Array.from(selected).map((id) => store.deleteResource("virtualParameters", id)))
                                    .then((res) => {
                                    notifications.push("success", `${res.length} virtual parameters deleted`);
                                    store.setTimestamp(Date.now());
                                })
                                    .catch((err) => {
                                    notifications.push("error", err.message);
            ...
            return [
                ...
                m("loading", { queries: [virtualParameters, count] }, m(indexTableComponent, attrs)),
            ];
        },
    };
};
