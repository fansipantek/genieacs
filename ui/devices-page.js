...
const getDownloadUrl = memoize((filter, indexParameters) => {
    ...
    for (const p of indexParameters)
        columns[p.label] = stringify(p.parameter);
...
function renderActions(selected) {
    ...
    buttons.push(m("button.critical", {
    ...
    buttons.push(m("button.primary", {
        ...
        onclick: () => {
            ...
            if (!confirm(`Deleting ${ids.length} devices. Are you sure?`))
                return;
            let counter = 1;
            for (const id of ids) {
                ++counter;
    ...
    buttons.push(m("button.primary", {
        ...
        onclick: () => {
            const ids = Array.from(selected);
            const tag = prompt(`Enter tag to unassign from ${ids.length} devices:`);
            if (!tag)
                return;
            let counter = 1;
            for (const id of ids) {
    ...
    return {
        view: (vnode) => {
            ...
            for (let i = 0; i < attributes.length; i++) {
                ...
                if (param)
                    sortAttributes[i] = sort[param] || 0;
            }
            function onSortChange(sortedAttrs) {
                const _sort = {};
                for (const index of sortedAttrs) {
                    const param = memoizedGetSortable(attributes[Math.abs(index) - 1].parameter);
            ...
            const valueCallback = (attr, device) => {
            ...
            attrs["recordActionsCallback"] = (device) => {
                return m("a", {
