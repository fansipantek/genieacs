        ...
        extract: (xhr) => {
            ...
            if (xhr.status !== 200) {
                if (!connectionNotification) {
                    connectionNotification = notifications.push("warning", "Server is unreachable", {});
                }
            }
            else {
                if (connectionNotification) {
                ...
                try {
                    ...
                    if (Math.abs(skew - clockSkew) > 5000 && now2 - now1 < 1000) {
                        clockSkew = skew;
                        console.warn(`System and server clocks are out of sync. Adding ${clockSkew}ms offset to any client-side time relative calculations.`);
                        setTimestamp(now2);
                        m.redraw();
                    }
                }
                ...
                if (!configNotification !== !configChanged) {
                    if (configNotification) {
                        ...
                        configNotification = notifications.push("warning", "Configuration has been modified, please reload the page", {
                            Reload: () => {
                ...
                if (!versionNotification !== !versionChanged) {
                    if (versionNotification) {
                        ...
                        versionNotification = notifications.push("warning", "Server has been updated, please reload the page", {
                            Reload: () => {
                                window.location.reload();
                            },
                        });
...
export async function xhrRequest(options) {
    const extract = options.extract;
    const deserialize = options.deserialize;
    options.extract = (xhr, _options) => {
        if (typeof extract === "function")
            return extract(xhr, _options);
...
export function unpackExpression(exp) {
    if (!Array.isArray(exp))
        return exp;
    const e = memoizedEvaluate(exp, null, fulfillTimestamp + clockSkew);
    return e;
}
export function count(resourceType, filter) {
...
function limitFilter(filter, sort, bookmark) {
    ...
    return and(filter, arr.reduce((cur, kv) => {
        const [param, asc] = kv;
        if (asc <= 0) {
            if (bookmark[param] == null) {
                return or(["IS NOT NULL", ["PARAM", param]], and(["IS NULL", ["PARAM", param]], cur));
            }
            let f = null;
...
function compareFunction(sort) {
    const sortEntries = Object.entries(sort).sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]));
    return (a, b) => {
...
function findMatches(resourceType, filter, sort, limit) {
    ...
    for (const obj of resources[resourceType].objects.values())
        if (evaluate(filter, obj, fulfillTimestamp + clockSkew))
            value.push(obj);
...
function inferQuery(resourceType, queryResponse) {
    ...
    if (bookmark || !limit) {
        if (bookmark)
            filter = limitFilter(filter, sort, bookmark);
        if (covers(resources[resourceType].combinedFilter, filter))
            queries.fulfilled.set(queryResponse, fulfillTimestamp);
    }
    queries.value.set(queryResponse, findMatches(resourceType, filter, sort, limit));
}
export function fetch(resourceType, filter, options = {}) {
    const filterStr = memoizedStringify(filter);
...
export function fulfill(accessTimestamp) {
    const allPromises = [];
    ...
    Promise.all(allPromises)
        ...
        for (const [resourceType, toFetch] of Object.entries(toFetchAll)) {
            ...
            const [union, diff] = unionDiff(resources[resourceType].combinedFilter, combinedFilter);
            if (!diff) {
                for (const queryResponse of toFetch) {
                    let filter = queries.filter.get(queryResponse);
                    filter = memoizedEvaluate(filter, null, fulfillTimestamp + clockSkew);
...
export function putResource(resourceType, id, object) {
...
export function queryConfig(pattern = "%") {
...
export function resourceExists(resource, id) {
...
export function evaluateExpression(exp, obj) {
    if (!Array.isArray(exp))
        return exp;
    return memoizedEvaluate(exp, obj, fulfillTimestamp + clockSkew);
}
export function changePassword(username, newPassword, authPassword) {
