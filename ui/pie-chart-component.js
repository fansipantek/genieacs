...
function drawChart(chartData) {
    ...
    const total = Array.from(Object.values(chartData.slices)).reduce((a, s) => a + (s["count"]["value"] || 0), 0);
    const legend = [];
    const paths = [];
    const links = [];
    ...
    for (const slice of Object.values(slices)) {
        ...
        if (percent > 0) {
            ...
            const sketch = `M ${startX} ${startY} ` + // Move to the starting point
            ...
            const percentageX = Math.cos(2 * Math.PI * (currentProgressPercentage - percent / 2)) * 50;
            const percentageY = Math.sin(2 * Math.PI * (currentProgressPercentage - percent / 2)) * 50;
            links.push(m("a", {
