...
function putActionHandler(action, _object, isNew) {
    return new Promise((resolve, reject) => {
        ...
        if (action === "save") {
            ...
            id = id.trim();
            if (!id.match(regex))
                return void resolve({ _id: "Invalid ID" });
            try {
...
function renderTable(confsResponse, searchString) {
    ...
    for (const conf of confs) {
        ...
        const del = m("button", {
            title: "Delete config",
            onclick: () => {
                if (!confirm(`Deleting ${conf._id} config. Are you sure?`))
                    return;
        ...
        rows.push(m("tr", attrs, m("td.left", m("long-text", { text: conf._id })), m("td.right", m("span", [m("long-text", { text: `${conf.value}` }), edit, del]))));
    }
    if (!rows.length)
...
export const component = () => {
    return {
        view: (vnode) => {
            ...
            if (window.authorizer.hasAccess("config", 3)) {
                ...
                for (const sub of subsData) {
                    ...
                    subs.push(m("button", {
                        onclick: () => {
                            let cb = null;
                            const comp = m(uiConfigComponent, Object.assign({
                                onUpdate: (errs) => {
                                    ...
                                    if (errors.length) {
                                        ...
                                        notifications.push("success", `${sub.name.replace(/^[a-z]/, sub.name[0].toUpperCase())} config updated`);
                                        overlay.close(cb);
                                    }
                                    store.setTimestamp(Date.now());
                                },
            ...
            return [
                m("h1", "Listing config"),
                m("loading", { queries: [confs] }, m(".all-parameters", search, m(".parameter-list", { style: "height: 400px" }, renderTable(confs, vnode.state["searchString"])), m(".actions-bar", [newConfig].concat(subs)))),
            ];
        },
    };
};
