...
const component = () => {
    return {
        ...
        view: (vnode) => {
            ...
            if (typeof object !== "string" || !device[object])
                return null;
            ...
            for (const i of instances) {
                ...
                const row = parameters.map((p) => {
                    ...
                    return m("td", m.context({
                        device: device,
                        parameter: param,
                    }, p.type || "parameter", Object.assign({}, p, {
                        device: device,
                        parameter: param,
                        label: null,
                    })));
            ...
            if (!rows.length) {
                rows.push(m("tr.empty", m("td", { colspan: headers.length }, "No instances")));
            }
            if (device[object].writable === true) {
                rows.push(m("tr", m("td", { colspan: headers.length }), m("td", m("button", {
            ...
            return [
                label,
                m("loading", { queries: [vnode.attrs["deviceQuery"]] }, m("table.table", thead, m("tbody", rows))),
            ];
        },
    };
};
