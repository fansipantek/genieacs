...
export function getLabels(resource) {
...
export function getTip(resource, label) {
    var _a;
    let tip;
    if ((_a = resources[resource]) === null || _a === void 0 ? void 0 : _a[label]) {
        const param = resources[resource][label];
        const types = resource === "devices" ? param["type"] : param["type"].split(",");
        const tips = [];
        for (const type of types) {
            switch (type.trim()) {
                ...
                case "number":
                    ...
                    break;
                case "timestamp":
                    tips.push("Unix timestamp or string in the form YYYY-MM-DDTHH:mm:ss.sssZ");
                    break;
                case "mac":
...
export function unpack(resource, label, value) {
    if (!resources[resource])
        return null;
