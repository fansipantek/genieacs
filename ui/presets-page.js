...
function putActionHandler(action, _object, isNew) {
    return new Promise((resolve, reject) => {
        ...
        if (action === "save") {
            ...
            const errors = {};
            if (!id)
                errors["_id"] = "ID can not be empty";
            if (!object.provision)
                errors["provision"] = "Provision not selected";
            if (Object.keys(errors).length)
                return void resolve(errors);
            ...
            store
                ...
                store
                    ...
                    .then(() => {
                    notifications.push("success", `Preset ${exists ? "updated" : "created"}`);
                    store.setTimestamp(Date.now());
                    resolve(null);
                })
                    .catch(reject);
...
export const component = () => {
    return {
        view: (vnode) => {
            ...
            const userDefinedProvisions = new Set();
            ...
            const valueCallback = (attr, preset) => {
                if (attr.id === "precondition") {
                    ...
                    return m("a", { href: devicesUrl, title: preset["precondition"] }, preset["precondition"]);
                }
                else if (attr.id === "provision" &&
            ...
            attrs["recordActionsCallback"] = (preset) => {
                return [
                    m("a", {
                        onclick: () => {
                            let cb = null;
                            const comp = m(putFormComponent, Object.assign({
                                base: preset,
                            ...
                            cb = () => {
                                if (!preset.provision) {
                                    return m("div", { style: "margin:20px" }, "This UI only supports presets with a single 'provision' configuration. If this preset was originally created from the old UI (genieacs-gui), you must edit it there.");
                                }
                                return comp;
                            };
                            overlay.open(cb, () => {
                                var _a;
                                return !((_a = comp.state) === null || _a === void 0 ? void 0 : _a["current"]["modified"]) ||
            ...
            if (window.authorizer.hasAccess("presets", 3)) {
                attrs["actionsCallback"] = (selected) => {
                    return [
                        m("button.primary", {
                            ...
                            onclick: () => {
                                ...
                                cb = () => comp;
                                overlay.open(cb, () => !comp.state["current"]["modified"] ||
                                    confirm("You have unsaved changes. Close anyway?"));
                            },
                        }, "New"),
                        m("button.primary", {
                            ...
                            onclick: (e) => {
                                ...
                                Promise.all(Array.from(selected).map((id) => store.deleteResource("presets", id)))
                                    .then((res) => {
            ...
            return [
                ...
                m("loading", { queries: [presets, count] }, m(indexTableComponent, attrs)),
            ];
        },
    };
};
