...
function createField(current, attr, focus) {
    if (attr.type === "combo") {
        ...
        const optionsValues = Array.from(new Set(attr.options.concat(current.object[attr.id] || [])));
            ...
            const opts = {
                ...
                oncreate: (_vnode) => {
                    if (focus && !options.length)
                        _vnode.dom.focus();
                    if (currentSelected.has(op))
                        _vnode.dom.checked = true;
                },
                onchange: (e) => {
                    if (e.target.checked)
    ...
    let datalist = null;
    if (attr.options)
        datalist = getDatalistId(attr.options);
    return m("input", {
        ...
        view: (vnode) => {
            ...
            for (const attr of attributes) {
                ...
                if (!focused && (current.isNew || attr.id !== "_id"))
                    focus = focused = true;
                form.push(m("p", m("label", { for: attr.id }, attr.label || attr.id), m("br"), createField(current, attr, focus)));
            }
            ...
            const children = [
                m("h1", `${current.isNew ? "New" : "Editing"} ${singular[resource] || resource}`),
                m("form", {
                    onsubmit: (e) => {
                        e.redraw = false;
