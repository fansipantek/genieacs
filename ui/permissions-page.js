...
function putActionHandler(action, _object) {
    return new Promise((resolve, reject) => {
        ...
        if (action === "save") {
            if (!object.role)
                return void resolve({ role: "Role can not be empty" });
            if (!object.resource)
                return void resolve({ resource: "Resource can not be empty" });
            if (!object.access)
                return void resolve({ access: "Access can not be empty" });
            if (object.access === "3: write")
                object.access = 3;
            else if (object.access === "2: read")
                object.access = 2;
            else if (object.access === "1: count")
                object.access = 1;
            else
                return void resolve({ access: "Invalid access level" });
            if (object.filter) {
                try {
...
export const component = () => {
    return {
        view: (vnode) => {
            ...
            const valueCallback = (attr, permission) => {
                if (attr.id === "access") {
                ...
                attrs["recordActionsCallback"] = (permission) => {
                    return [
                        m("button", {
                            ...
                            onclick: () => {
                                if (!confirm(`Deleting ${permission._id} permission. Are you sure?`))
                                    return;
                ...
                attrs["actionsCallback"] = (selected) => {
                    return [
                        ...
                        m("button.primary", {
                            ...
                            onclick: (e) => {
                                if (!confirm(`Deleting ${selected.size} permissions. Are you sure?`))
                                    return;
                                e.redraw = false;
                                e.target.disabled = true;
                                Promise.all(Array.from(selected).map((id) => store.deleteResource("permissions", id)))
                                    .then((res) => {
                                    notifications.push("success", `${res.length} permissions deleted`);
                                    store.setTimestamp(Date.now());
                                })
                                    .catch((err) => {
                                    notifications.push("error", err.message);
