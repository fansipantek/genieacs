...
export const component = () => {
    return {
        view: (vnode) => {
            ...
            const valueCallback = (attr, fault) => {
                if (attr.id === "device") {
                    ...
                    return m("button.primary", {
                        ...
                        onclick: (e) => {
                            ...
                            if (!confirm(`Deleting ${selected.size} faults. Are you sure?`))
                                return;
                            Promise.all(Array.from(selected).map((id) => store.deleteResource("faults", id)))
                                .then((res) => {
            ...
            return [
                ...
                m("loading", { queries: [faults, count] }, m(indexTableComponent, attrs)),
            ];
        },
    };
};
