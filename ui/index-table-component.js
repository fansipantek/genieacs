...
function renderTable(attributes, data, total, showMoreCallback, selected, sortAttributes, onSort, downloadUrl, valueCallback, actionsCallback, recordActionsCallback) {
    const records = data || [];
    // Actions bar
    let buttons = [];
    ...
    for (let i = 0; i < attributes.length; i++) {
        ...
        if (sortAttributes[i] > 0)
            symbol = getIcon("sorted-asc");
        else if (sortAttributes[i] < 0)
            symbol = getIcon("sorted-dsc");
        const sortable = m("button", {
            onclick: (e) => {
                e.redraw = false;
                onSort(i);
            },
    ...
    for (const record of records) {
        ...
        if (typeof recordActionsCallback === "function") {
            ...
            if (!Array.isArray(recordButtons))
    ...
    if (total != null)
        footerElements.push(`${records.length}/${total}`);
    else
        footerElements.push(`${records.length}`);
    footerElements.push(m("button", {
        title: "Show more records",
        onclick: showMoreCallback,
        disabled: !data.length || records.length >= Math.min(MAX_PAGE_SIZE, total),
    }, "More"));
    if (downloadUrl) {
        footerElements.push(m("a.download-csv", { href: downloadUrl, download: "" }, "Download"));
    }
    const tfoot = m("tfoot", m("tr", m("td", { colspan: labels.length }, footerElements)));
    const children = [
        m("table.table.highlight", m("thead", m("tr", labels)), m("tbody", rows), tfoot),
    ];
    if (buttons.length)
        children.push(m("div.actions-bar", buttons));
    return children;
}
const component = () => {
    let selected = new Set();
    let sortingfunction;
    const onSort = debounce((events) => {
    ...
    return {
        view: (vnode) => {
            ...
            sortingfunction = (events) => {
                const sortArray = new Set(Object.keys(sortAttributes)
                    .map((x) => (parseInt(x) + 1) * sortAttributes[x])
                    .filter((x) => x));
                for (const num of events) {
                    if (sortArray.delete(num + 1))
                        sortArray.add(-(num + 1));
                    else if (!sortArray.delete((num + 1) * -1))
                        sortArray.add(num + 1);
                }
                onSortChange(Array.from(sortArray).reverse());
            };
            selected = _selected;
            return renderTable(attributes, data, total, showMoreCallback, selected, sortAttributes, onSort, downloadUrl, valueCallback, actionsCallback, recordActionsCallback);
        },
    };
};
