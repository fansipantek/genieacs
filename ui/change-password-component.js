...
const component = () => {
    return {
        view: (vnode) => {
            ...
            const form = [
                m("p", m("label", { for: "username" }, "Username"), m("input", {
                    name: "username",
                    type: "text",
            ...
            for (const [f, l] of Object.entries(fields)) {
                form.push(m("p", m("label", { for: f }, l), m("input", {
                    name: f,
                    type: "password",
                    value: vnode.state[f],
                    oninput: (e) => {
            ...
            const children = [
                ...
                m("form", {
                    onsubmit: (e) => {
                        e.redraw = false;
                        e.preventDefault();
                        if (!vnode.state["username"] ||
                            !vnode.state["newPassword"] ||
                            (enforceAuth && !vnode.state["authPassword"])) {
                            notifications.push("error", "Please fill all fields");
                        }
                        else if (vnode.state["newPassword"] !== vnode.state["confirmPassword"]) {
                            notifications.push("error", "Password confirm doesn't match new password");
                        }
                        else {
                            submit.dom.disabled = true;
                            changePassword(vnode.state["username"], vnode.state["newPassword"], vnode.state["authPassword"])
                                .then(() => {
