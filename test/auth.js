...
ava("digest", (t) => {
    ...
    for (const challenge of challenges) {
        ...
        const solution = auth.solveDigest(username, password, uri, method, body, wwwAuthHeader);
        const authHeader = auth.parseAuthorizationHeader(solution);
        t.is(authHeader["response"], auth.digest(username, realm, password, nonce, method, uri, authHeader["qop"], body, authHeader["cnonce"], authHeader["nc"]));
    }
});
