...
test.before(async (t) => {
    ...
    t.context.query = async (filter) => {
        const res = db.exec(`SELECT id FROM test WHERE ${filter}`);
        if (!res.length)
            return new Set();
        return new Set(res[0].values.flat());
    };
    db.run("CREATE TABLE test (id INTEGER PRIMARY KEY, string STRING, decimal DECIMAL(4,2))");
...
test("minimize", async (t) => {
    ...
    for (const [s1, s2, s3, op1, op2] of getPermutations(STRING_VALUES.filter((s) => s), STRING_VALUES.filter((s) => s), STRING_VALUES.filter((s) => s), [">", "=", "<"], ["<>", ">="])) {
        cases.push(`string ${op1} "${s1}" OR string ='${s2}' OR NOT string ${op2} '${s3}'`);
    }
    for (const [s1, s2] of getPermutations(STRING_VALUES.filter((s) => s), STRING_VALUES.filter((s) => s)))
