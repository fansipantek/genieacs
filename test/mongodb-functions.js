...
ava("convertOldPrecondition", (t) => {
    ...
    for (const test of shouldFailTests) {
        const func = () => {
            mongodbFunctions.convertOldPrecondition(test[0]);
        };
...
ava("filterToMongoQuery", async (t) => {
    ...
    for (const [expStr, expect] of queries) {
        ...
        const query = mongodbFunctions.filterToMongoQuery(mongodbFunctions.processDeviceFilter(exp));
    ...
    for (const [expStr, err] of failQueries) {
        const exp = parse(expStr);
        t.throws(() => mongodbFunctions.filterToMongoQuery(mongodbFunctions.processDeviceFilter(exp)), { message: err });
    }
});
