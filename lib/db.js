...
export let tasksCollection, devicesCollection, presetsCollection, objectsCollection, provisionsCollection, virtualParametersCollection, faultsCollection, filesCollection, operationsCollection, permissionsCollection, usersCollection, configCollection;
let clientPromise;
...
export async function fetchDevice(id, timestamp) {
    const res = [
        [
            Path.parse("Events"),
            timestamp,
    ...
    if (!device)
        return null;
    function storeParams(obj, path, pathLength, ts) {
        if (obj["_timestamp"])
    ...
    const ts = device["_timestamp"] || 0;
    if (ts)
        res.push([Path.parse("*"), ts]);
    for (const [k, v] of Object.entries(device)) {
        switch (k) {
            ...
            default:
                if (!k.startsWith("_"))
                    storeParams(v, k, 1, ts);
        }
    }
    return res;
}
export async function saveDevice(deviceId, deviceData, isNew, sessionTimestamp) {
    var _a, _b, _c, _d;
    const update = { $set: {}, $unset: {}, $addToSet: {}, $pull: {} };
    for (const diff of deviceData.timestamps.diff()) {
        if (diff[0].wildcard !== 1 << (diff[0].length - 1))
            continue;
        if (diff[0].segments[0] === "Events" ||
        ...
        if (diff[2] == null && diff[1] != null) {
            update["$unset"][parent.length ? parent.toString() + "._timestamp" : "_timestamp"] = 1;
        }
        else {
            if (parent && (!parent.length || deviceData.attributes.has(parent))) {
                update["$set"][parent.length ? parent.toString() + "._timestamp" : "_timestamp"] = new Date(diff[2]);
            }
        }
    }
            ...
            case "Tags":
                if (value2 !== value1) {
                    if (value2 != null) {
                        ...
                        update["$addToSet"]["_tags"]["$each"].push(decodeTag(path.segments[1]));
                    }
                    else {
                        ...
                        update["$pull"]["_tags"]["$in"].push(decodeTag(path.segments[1]));
                    }
                }
                break;
            default:
                if (!diff[2]) {
                    ...
                    if (pathStr.endsWith(INVALID_PATH_SUFFIX)) {
                        ...
                        splits[splits.length - 1] = decodeTag(splits[splits.length - 1].slice(0, 0 - INVALID_PATH_SUFFIX.length));
                        pathStr = splits.join(".");
                    }
                ...
                for (const attrName of Object.keys(diff[2])) {
                    ...
                    if (diff[2][attrName][1] != null) {
                        switch (attrName) {
                            case "value":
                                if (value2 !== value1) {
                                    if (valueType2 === "xsd:dateTime" &&
                                        Number.isInteger(value2)) {
                                        update["$set"][path.toString() + "._value"] = new Date(value2);
                                    }
                                    else {
                                ...
                                if (valueTimestamp2 !== valueTimestamp1) {
                                    update["$set"][path.toString() + "._timestamp"] = new Date(valueTimestamp2);
                                }
                                break;
                            case "object":
                                if (!((_a = diff[1]) === null || _a === void 0 ? void 0 : _a.object) || object2 !== object1) {
                                    update["$set"][path.length ? path.toString() + "._object" : "_object"] = !!object2;
                                }
                                break;
                            case "writable":
                                if (!((_b = diff[1]) === null || _b === void 0 ? void 0 : _b.writable) || writable2 !== writable1) {
                                    update["$set"][path.length ? path.toString() + "._writable" : "_writable"] = !!writable2;
                                }
                                break;
                            case "notification":
                                if (!diff[1] ||
                                    ...
                                    update["$set"][path.length
                                        ? path.toString() + "._notification"
                                        : "_notification"] = notification2;
                                }
                ...
                if (diff[1]) {
                    for (const attrName of Object.keys(diff[1])) {
                        if (diff[1][attrName][1] != null &&
                            ((_d = (_c = diff[2]) === null || _c === void 0 ? void 0 : _c[attrName]) === null || _d === void 0 ? void 0 : _d[1]) == null) {
...
export async function getFaults(deviceId) {
...
export async function saveFault(deviceId, channel, fault) {
...
export async function clearTasks(deviceId, taskIds) {
    await tasksCollection.deleteMany({
        _id: { $in: taskIds.map((id) => new ObjectId(id)) },
    });
}
export async function getOperations(deviceId) {
...
export async function saveOperation(deviceId, commandKey, operation) {
...
export async function deleteOperation(deviceId, commandKey) {
...
export async function getVirtualParameters() {
