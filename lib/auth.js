...
export function parseAuthorizationHeader(authHeader) {
    ...
    if (method === "Basic") {
        ...
        const creds = USER_PASS_REGEX.exec(Buffer.from(authHeader.slice(method.length + 1), "base64").toString());
        if (!creds)
            throw new Error("Unable to parse auth header");
...
export function digest(username, realm, password, nonce, httpMethod, uri, qop, body, cnonce, nc) {
...
export function solveDigest(username, password, uri, httpMethod, body, authHeader) {
    ...
    const hash = digest(username, authHeader.realm, password, authHeader.nonce, httpMethod, uri, qop, body, cnonce, nc);
