    ...
    return map(filter, (exp) => {
        ...
        if (exp[0] === "PARAM") {
            ...
            if (p === "DeviceID.ID")
                ...
                return ["PARAM", "_deviceId"];
            else if (p.startsWith("DeviceID."))
                return ["PARAM", "_deviceId._" + p.slice(9)];
            else if (p === "Events.Inform")
                return ["PARAM", "_lastInform"];
            else if (p === "Events.Registered")
                return ["PARAM", "_registered"];
            else if (p === "Events.0_BOOTSTRAP")
                return ["PARAM", "_lastBootstrap"];
            else if (p === "Events.1_BOOT")
                return ["PARAM", "_lastBoot"];
        }
        else if (isArray(exp[1]) &&
            exp[1][0] === "PARAM" &&
            exp[1][1].startsWith("Tags.")) {
            const t = decodeTag(exp[1][1].slice(5));
            if (exp[0] === "IS NULL")
                return ["_tags", t, false];
...
export function processFaultsFilter(filter) {
    return map(filter, (exp) => {
        ...
        if (["=", "<>", ">", ">=", "<", "<="].includes(exp[0])) {
            const e = exp.slice();
            if (e[1][0] === "PARAM" && e[1][1] === "timestamp")
                e[2] = new Date(e[2]);
            else if (e[1][0] === "PARAM" && e[1][1] === "expiry")
                e[2] = new Date(e[2]);
            return e;
...
export function filterToMongoQuery(exp) {
    function recursive(filter, negate) {
        const op = filter[0];
        if (op === "AND") {
            filter = filter.filter((f) => f !== true);
            if (filter.length === 2)
                return recursive(filter[1], negate);
        }
        if (op === "OR") {
        ...
        if (op === "_tags") {
            ...
            if (negate)
                t = !t;
            if (t)
                return { _tags: filter[1] };
            else
                return { _tags: { $ne: filter[1] } };
        }
...
export function flattenDevice(device) {
    function recursive(input, root, output, timestamp) {
        for (const [name, tree] of Object.entries(input)) {
            ...
            if (!root)
                childrenTimestamp = +(input["_timestamp"] || 1);
...
export function convertOldPrecondition(query) {
            ...
            if (k[0] === "$") {
                if (k === "$and") {
                    ...
                    for (const vv of Object.values(v))
                        and.push(recursive(vv));
                    expressions.push(and);
                }
                else if (k === "$or") {
                    const or = ["OR"];
                    for (const vv of Object.values(v))
                        or.push(recursive(vv));
                    expressions.push(or);
                }
                else {
                ...
                if (typeof v === "object") {
                    ...
                    if (conjs.length === 1)
                        expressions.push(conjs[0]);
                    else if (conjs.length > 1)
                        expressions.push(["AND", ...conjs]);
                }
                else {
                    expressions.push([
                        "IS NOT NULL",
                ...
                if (typeof v === "boolean")
                    ...
                    exists = !v["$ne"];
                else if (v.hasOwnProperty("$exists"))
                    exists = !!v["$exists"];
                else
                    throw new Error(`Invalid tag query`);
...
export function flattenPreset(preset) {
    ...
    if (p.precondition) {
        try {
            ...
            p.precondition = convertOldPrecondition(JSON.parse(p.precondition));
            p.precondition = p.precondition.length
                ? stringify(p.precondition)
                : "";
        }
    }
    if (p.events) {
