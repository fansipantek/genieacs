...
onConnect(async (db) => {
    for (const k of Object.keys(collections)) {
        if (k === "files")
            collections[k] = db.collection("fs.files");
        else
            collections[k] = db.collection(k);
    }
...
export function listener(request, response) {
    ...
    request.addListener("end", () => {
        ...
        logger.accessInfo(Object.assign({}, urlParts.query, {
        ...
        if (PRESETS_REGEX.test(urlParts.pathname)) {
            const presetName = decodeURIComponent(PRESETS_REGEX.exec(urlParts.pathname)[1]);
            if (request.method === "PUT") {
                ...
                collections.presets.replaceOne({ _id: presetName }, preset, { upsert: true }, (err) => {
                    if (err)
            ...
            const objectName = decodeURIComponent(OBJECTS_REGEX.exec(urlParts.pathname)[1]);
            if (request.method === "PUT") {
                let object;
                try {
                ...
                collections.objects.replaceOne({ _id: objectName }, object, { upsert: true }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    cache
                        .del("presets_hash")
            ...
            const provisionName = decodeURIComponent(PROVISIONS_REGEX.exec(urlParts.pathname)[1]);
            if (request.method === "PUT") {
                const object = {
                    _id: provisionName,
                ...
                collections.provisions.replaceOne({ _id: provisionName }, object, { upsert: true }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    cache
                        .del("presets_hash")
            ...
            const virtualParameterName = decodeURIComponent(VIRTUAL_PARAMETERS_REGEX.exec(urlParts.pathname)[1]);
            if (request.method === "PUT") {
                const object = {
                ...
                collections.virtualParameters.replaceOne({ _id: virtualParameterName }, object, { upsert: true }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    cache
                        .del("presets_hash")
                ...
                collections.virtualParameters.deleteOne({ _id: virtualParameterName }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    cache
                        .del("presets_hash")
            ...
            if (request.method === "POST") {
                collections.devices.updateOne({ _id: deviceId }, { $addToSet: { _tags: tag } }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    response.writeHead(200);
                    response.end();
                ...
                collections.devices.updateOne({ _id: deviceId }, { $pull: { _tags: tag } }, (err) => {
                    if (err)
                        return void throwError(err, response);
                    response.writeHead(200);
                    response.end();
            ...
            if (request.method === "DELETE") {
                const faultId = decodeURIComponent(FAULTS_REGEX.exec(urlParts.pathname)[1]);
            ...
            if (request.method === "POST") {
                const deviceId = decodeURIComponent(DEVICE_TASKS_REGEX.exec(urlParts.pathname)[1]);
                if (body.length) {
                    let task;
                    try {
                    ...
                    collections.devices
                        ...
                        const notInSession = await apiFunctions.awaitSessionEnd(deviceId, 30000);
                        await cache.del(`${deviceId}_tasks_faults_operations`);
                        if (urlParts.query.connection_request == null) {
                            if (socketTimeout)
                                request.socket.setTimeout(socketTimeout);
                        ...
                        const onlineThreshold = (urlParts.query.timeout &&
                            parseInt(urlParts.query.timeout)) ||
                            config.get("DEVICE_ONLINE_THRESHOLD", deviceId);
                        const sessionStarted = await apiFunctions.awaitSessionStart(deviceId, lastInform, onlineThreshold);
                        if (!sessionStarted) {
                            if (socketTimeout)
                                request.socket.setTimeout(socketTimeout);
                            response.writeHead(202, "Task queued but not processed", {
                        ...
                        const prom1 = collections.tasks.findOne({ _id: task._id }, { projection: { _id: 1 } });
                        const prom2 = collections.faults.findOne({ _id: `${deviceId}:task_${task._id}` }, {
                            projection: { _id: 1 },
                        });
            ...
            if (!action || action === "/") {
                if (request.method === "DELETE") {
                    collections.tasks.findOne({ _id: new ObjectId(taskId) }, { projection: { device: 1 } }, (err, task) => {
                        if (err)
                            return void throwError(err, response);
                        if (!task) {
                            response.writeHead(404);
                        ...
                        collections.tasks.deleteOne({ _id: new ObjectId(taskId) }, (err) => {
                            if (err)
                                return void throwError(err, response);
                            collections.faults.deleteOne({ _id: `${deviceId}:task_${taskId}` }, (err) => {
                                if (err)
                                    return void throwError(err, response);
                                cache
                                    .del(`${deviceId}_tasks_faults_operations`)
            ...
            const filename = decodeURIComponent(FILES_REGEX.exec(urlParts.pathname)[1]);
            if (request.method === "PUT") {
                const metadata = {
                ...
                filesBucket.delete(filename, () => {
                    const uploadStream = filesBucket.openUploadStreamWithId(filename, filename, {
                        metadata: metadata,
                    });
                ...
                filesBucket.delete(filename, (err) => {
                    if (err) {
            ...
            const deviceId = decodeURIComponent(DELETE_DEVICE_REGEX.exec(urlParts.pathname)[1]);
            apiFunctions
            ...
            collection.countDocuments(q, (err, total) => {
                ...
                if (urlParts.query.limit)
                    cur.limit(parseInt(urlParts.query.limit));
                response.write("[\n");
                i = 0;
                cur.forEach((item) => {
                    if (i++)
