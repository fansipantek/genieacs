...
import { parseXml, parseAttrs } from "./xml-parser";
...
function detectStreamTag(data) {
    ...
    if (i1 < 0)
        throw new Error("Cannot detect opening stream tag");
...
function saltPassword(password, saltBase64, iteractionCount) {
    if (password === saltedPasswordCache.password &&
...
function* loginPlain(socket, username, password) {
    socket.write(`<auth xmlns="urn:ietf:params:xml:ns:xmpp-sasl" mechanism="PLAIN">${encodeBase64(`\x00${username}\x00${password}`)}</auth>`);
    const res1 = yield;
    if (res1.name === "failure" &&
...
function* loginScram(socket, username, password) {
    ...
    socket.write(`<auth xmlns="urn:ietf:params:xml:ns:xmpp-sasl" mechanism="SCRAM-SHA-1">${encodeBase64(clientFirstMessage)}</auth>`);
    const res1 = yield;
    ...
    for (const s of serverFirstMessage.split(",")) {
        if (s.startsWith("i="))
            iterationCount = parseInt(s.slice(2));
        else if (s.startsWith("s="))
            saltBase64 = s.slice(2);
    ...
    const clientFinalMessageWithoutProof = `c=${encodeBase64(gs2Header)},r=${nonce}`;
    const authMessage = `${clientFirstMessageBare},${serverFirstMessage},${clientFinalMessageWithoutProof}`;
    const clientSignature = createHmac("sha1", storedKey)
        .update(authMessage)
        .digest();
    const clientProof = Buffer.from(clientKey);
    for (const [i, b] of clientSignature.entries())
        clientProof[i] ^= b;
    const clientFinalMessage = `${clientFinalMessageWithoutProof},p=${clientProof.toString("base64")}`;
    socket.write(`<response xmlns="urn:ietf:params:xml:ns:xmpp-sasl">${encodeBase64(clientFinalMessage)}</response>`);
    const res2 = yield;
    if (res2.name === "failure" &&
...
function* starttls(socket) {
    ...
    const res1 = yield;
    if (res1.name !== "proceed")
        throw new Error("Failed to initiate STARTTLS");
}
function* bind(socket, resource) {
    const id = randomBytes(8).toString("base64");
    socket.write(`<iq id='${id}' type='set'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><resource>${resource}</resource></bind></iq>`);
    const res1 = yield;
    if (res1.name !== "iq")
        throw new Error(`Unexpected element ${res1.name}`);
    ...
    if (typeAttr.value !== "result")
        throw new Error("Cannot bind to resource");
}
const STATUS_RESTART_STREAM = 1;
const STATUS_STARTTLS = 2;
function* init(socket, host, username, password, resource) {
    socket.write(`<?xml version='1.0'?><stream:stream from='${username}@${host}' to='${host}' version='1.0' xml:lang='en' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'>`);
    const open = yield;
    ...
    for (const feature of features.children) {
        if (feature.name === "starttls") {
            ...
            const mechanisms = new Set(feature.children.map((c) => c.text));
...
export default class XmppClient extends EventEmitter {
            ...
            socket.connect(opts.port || 5222, opts.host, async () => {
                ...
                try {
                    ...
                    while (status) {
                        ...
                        status = await xmppStream(socket, init(socket, opts.host, opts.username, opts.password, opts.resource));
                    }
    ...
    _onError(err) {
        this._socket.end();
        this.emit("error", err);
        for (const cb of this._iqStanzaCallbacks.values())
            cb(err);
    }
    send(msg) {
        this._socket.write(msg);
    }
    sendIqStanza(from, to, type, body, timeout = 3000) {
        return new Promise((resolve, reject) => {
            const id = randomBytes(8).toString("base64");
            ...
            const t = setTimeout(() => {
                ...
                reject(new Error("Did not receive IQ stanza response in a timely manner"));
            }, timeout);
