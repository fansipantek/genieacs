...
export function query(resource, filter, options, callback) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (options.projection) {
            ...
            if (resource === "presets")
                projection.configurations = 1;
            projection = optimizeProjection(projection);
            cursor.project(projection);
        }
        if (resource === "users")
            cursor.project({ password: 0, salt: 0 });
        if (options.skip)
        ...
        if (options.sort) {
            let s = Object.entries(options.sort)
                ...
                .reduce((obj, [k, v]) => Object.assign(obj, { [k]: Math.min(Math.max(v, -1), 1) }), {});
            if (resource === "devices")
                s = mongodbFunctions.processDeviceSort(s);
            cursor.sort(s);
        }
        if (!callback) {
...
export async function updateDeviceTags(deviceId, tags) {
    ...
    if (add === null || add === void 0 ? void 0 : add.length)
        object["$addToSet"] = { _tags: { $each: add } };
    if (pull === null || pull === void 0 ? void 0 : pull.length)
        object["$pullAll"] = { _tags: pull };
...
export function putProvision(id, object) {
    ...
    try {
        ...
        if ((_a = err.stack) === null || _a === void 0 ? void 0 : _a.startsWith(`${id}:`)) {
            return Promise.reject(new Error(`${err.name} at ${err.stack.split("\n", 1)[0]}`));
        }
