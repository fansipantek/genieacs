...
router.get(`/devices/:id.csv`, async (ctx) => {
    ...
    ctx.type = "text/csv";
    ctx.attachment(`device-${ctx.params.id}-${new Date()
        .toISOString()
        .replace(/[:.]/g, "")}.csv`);
    ctx.body = new PassThrough();
    ctx.body.write("Parameter,Object,Object timestamp,Writable,Writable timestamp,Value,Value type,Value timestamp,Notification,Notification timestamp,Access list,Access list timestamp\n");
...
for (const [resource, flags] of Object.entries(resources)) {
    ...
    router.get(`/${resource}`, async (ctx) => {
        ...
        if (ctx.request.query.limit)
            options.limit = +ctx.request.query.limit;
        if (ctx.request.query.skip)
            options.skip = +ctx.request.query.skip;
    ...
    router.get(`/${resource}.csv`, async (ctx) => {
        ...
        const columns = JSON.parse(singleParam(ctx.request.query.columns));
        const now = Date.now();
        ...
        ctx.body = new PassThrough();
        ctx.type = "text/csv";
        ctx.attachment(`${resource}-${new Date(now).toISOString().replace(/[:.]/g, "")}.csv`);
        ctx.body.write(Object.keys(columns).map((k) => `"${k.replace(/"/, '""')}"`) + "\n");
        db.query(resource, filter, options, (obj) => {
                    ...
                    if (Array.isArray(e)) {
                        if (e[0] === "PARAM") {
                            if (resource === "devices") {
                                ...
                                if (e[1] === "detail")
                                    return yamlStringify(obj["detail"]);
                            }
                        }
                        else if (e[0] === "FUNC") {
                            if (e[1] === "DATE_STRING") {
...
router.post("/devices/:id/tasks", async (ctx) => {
    ...
    const onlineThreshold = getConfig(ctx.state.configSnapshot, "cwmp.deviceOnlineThreshold", {}, Date.now(), (exp) => {
        ...
        if (exp[0] === "PARAM") {
            ...
            if (exp[1] === "REMOTE_ADDRESS") {
                for (const root of ["InternetGatewayDevice", "Device"]) {
                    ...
                    if (p === null || p === void 0 ? void 0 : p.value)
                        return new URL(p.value[0]).hostname;
                }
                return null;
            }
        }
    ...
    const res = await apiFunctions.postTasks(ctx.params.id, ctx.request.body, onlineThreshold, device);
    // Restore socket timeout
    if (socketTimeout)
        ctx.socket.setTimeout(socketTimeout);
...
router.put("/users/:id/password", async (ctx) => {
    ...
    if (!ctx.state.user) {
        // User not logged in
        if (!(await apiFunctions.authLocal(ctx.state.configSnapshot, username, ctx.request.body.authPassword))) {
            logUnauthorizedWarning(log);
            ctx.status = 401;
