...
export function getAliasDeclarations(path, timestamp, attrGet = null) {
        ...
        for (const [i, alias] of path.segments.entries()) {
            if (Array.isArray(alias)) {
                ...
                for (const [p] of alias) {
                    decs = decs.concat(getAliasDeclarations(parent.concat(p), timestamp, {
                        value: timestamp,
                    }));
                }
            }
...
export function unpack(deviceData, path, revision) {
    ...
    if (!path.alias) {
        ...
        for (let i = path.length - 1; i >= 0; --i) {
            if (path.alias & (1 << i)) {
                for (const [param, val] of path.segments[i]) {
                        ...
                        if (attributes &&
                            ...
                            for (let m = 0; m < allMatches.length; ++m) {
                                ...
                                if (!match)
                                    continue;
                                for (k = i; k >= 0; --k)
                                    if (match.segments[k] !== up.segments[k])
                                        break;
                                if (k < 0) {
                                    filtered.push(match);
                                    allMatches[m] = null;
    ...
    allMatches.sort((p1, p2) => {
        for (let i = 0; i < p1.length; ++i) {
            const a = p1.segments[i];
            const b = p2.segments[i];
            if (a !== b) {
...
export function clear(deviceData, path, timestamp, attributes, changeFlags = 0) {
    const changeTrackers = {};
    timestamp = timestamp || 0;
    let descendantsTimestamp = timestamp;
    if (attributes === null || attributes === void 0 ? void 0 : attributes.object) {
    ...
    for (const p of deviceData.paths.find(path, true, true, descendantsTimestamp ? 99 : path.length)) {
...
export function set(deviceData, path, timestamp, attributes, toClear) {
    ...
    if (attributes) {
        ...
        if (!(timestamp <= currentTimestamp)) {
            ...
            if (path.length > 1) {
                toClear = set(deviceData, path.slice(0, path.length - 1), timestamp, { object: [timestamp, 1] }, toClear);
            }
        }
    }
