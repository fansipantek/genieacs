...
import { encodeEntities, parseAttrs } from "./xml-parser";
import * as logger from "../lib/logger";
async function extractAuth(exp, dflt) {
    let username, password;
    const _exp = await evaluateAsync(exp, {}, 0, async (e) => {
...
function httpGet(url, options, _debug, deviceId) {
...
export async function httpConnectionRequest(address, authExp, allowBasicAuth, timeout, _debug, deviceId) {
    ...
    while (!authHeader || (username != null && password != null)) {
        ...
        if (authHeader) {
            if (authHeader["method"] === "Basic") {
                if (!allowBasicAuth)
                    return "Basic HTTP authentication not allowed";
                opts = Object.assign({
                    headers: {
                ...
                opts = Object.assign({
                    headers: {
                        Authorization: auth.solveDigest(username, password, url.pathname + url.search, "GET", null, authHeader),
                    },
                }, options);
            }
            else {
...
export async function udpConnectionRequest(host, port, authExp, sourcePort = 0, _debug, deviceId) {
    const now = Date.now();
    ...
    if (sourcePort)
        client.bind({ port: sourcePort, exclusive: true });
    let username;
    let password;
            ...
            await new Promise((resolve, reject) => {
                client.send(message, 0, message.length, port, host, (err) => {
                    if (err)
                        ...
                        resolve();
                    if (_debug)
                        debug.outgoingUdpMessage(host, deviceId, port, msg);
                });
            });
        }
        [username, password, authExp] = await extractAuth(authExp, null);
...
const XMPP_JID = config.get("XMPP_JID");
const XMPP_PASSWORD = config.get("XMPP_PASSWORD");
...
export async function xmppConnectionRequest(jid, authExp, timeout, _debug, deviceId) {
    if (!xmppClient) {
    ...
    [username, password, authExp] = await extractAuth(authExp, null);
    while (username != null && password != null) {
        const msg = `<connectionRequest xmlns="urn:broadband-forum-org:cwmp:xmppConnReq-1-0"><username>${encodeEntities(username)}</username><password>${encodeEntities(password)}</password></connectionRequest>`;
        let res, rawRes, rawReq;
        try {
            ({ res, rawRes, rawReq } = await xmppClient.sendIqStanza(XMPP_JID, jid, "get", msg, timeout));
        }
        catch (err) {
            return err.message;
        }
