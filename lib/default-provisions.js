...
export function refresh(sessionContext, provision, declarations) {
    if ((provision.length !== 2 || typeof provision[1] !== "string") &&
        (provision.length !== 3 ||
    ...
    if (provision[3] == null) {
        ...
        for (const a of provision.slice(4))
            attrGet[a] = t;
    }
...
export function instances(sessionContext, provision, declarations, startRevision, endRevision) {
    ...
    if (provision[2][0] === "+" || provision[2][0] === "-") {
        ...
        if (endRevision === startRevision)
            return false;
        const unpacked = device.unpack(sessionContext.deviceData, path, startRevision + 1);
