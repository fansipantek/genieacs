...
function setConfig(name, value, commandLineArgument = false) {
    ...
    if (name === "GET_PARAMETER_NAMES_DEPTH_THRESHOLD" ||
        name === "get-parameter-names-depth-threshold")
        setConfig("GPN_NEXT_LEVEL", value);
    if (name === "TASK_PARAMETERS_BATCH_SIZE" ||
...
if (configDir)
    setConfig("EXT_DIR", `${configDir}/ext`);
...
if (fsHostname) {
    ...
    setConfig("FS_URL_PREFIX", (FS_SSL ? "https" : "http") + `://${fsHostname}:${FS_PORT}/`);
}
// Defaults
