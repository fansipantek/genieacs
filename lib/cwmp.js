...
const sessionsNonces = new WeakMap();
const stats = {
...
async function authenticate(sessionContext, body) {
    const authExpression = localCache.getConfigExpression(sessionContext.cacheSnapshot, "cwmp.auth");
    ...
    if ((authentication === null || authentication === void 0 ? void 0 : authentication.method) === "Digest") {
    ...
    const res = await evaluateAsync(authExpression, {}, sessionContext.timestamp, async (e) => {
        e = session.configContextCallback(sessionContext, e);
        if (Array.isArray(e) && e[0] === "FUNC") {
            if (e[1] === "EXT") {
                if (typeof e[2] !== "string" || typeof e[3] !== "string")
                    return null;
                ...
                if (username != null && password != null && authentication) {
                    if (authentication["method"] === "Basic") {
                        return (authentication["username"] === e[2] &&
                            authentication["password"] === e[3]);
                    }
                    if (authentication["method"] === "Digest") {
                        const expected = auth.digest(username, REALM, password, authentication["nonce"], "POST", authentication["uri"], authentication["qop"], authentication["body"], authentication["cnonce"], authentication["nc"]);
...
async function writeResponse(sessionContext, res, close = false) {
    // Close connection after last request in session
    if (close)
        res.headers["Connection"] = "close";
    let data = res.data;
    // Respond using the same content-encoding as the request
    if (sessionContext.httpRequest.headers["content-encoding"] &&
        res.data.length > 0) {
    ...
    if (connection.destroyed) {
        ...
        if (now >= sessionContext.extendLock) {
            ...
            const lockToken = await cache.acquireLock(`cwmp_session_${sessionContext.deviceId}`, sessionContext.timeout * 1000 + 15000, 0, sessionContext.sessionId);
            if (!lockToken)
...
function recordFault(sessionContext, fault, provisions, channels) {
    ...
    for (const channel of channelKeys) {
        const provs = sessionContext.faults[channel]
            ...
            : [];
        faults[channel] = Object.assign({ provisions: provs, timestamp: sessionContext.timestamp }, fault);
...
async function inform(sessionContext, rpc) {
    ...
    const cookiesPath = localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.cookiesPath", {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e));
    if (cookiesPath) {
...
async function transferComplete(sessionContext, rpc) {
...
async function applyPresets(sessionContext) {
    var _a;
    ...
    const RETRY_DELAY = +localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.retryDelay", {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e));
    ...
    for (const p of deviceData.paths.find(Path.parse("Events.*"), false, true)) {
        ...
        if ((attrs === null || attrs === void 0 ? void 0 : attrs.value) && attrs.value[1][0] >= sessionContext.timestamp)
            deviceEvents[p.segments[1]] = true;
    }
    const parameters = {};
    ...
    for (const preset of presets) {
        ...
        if (!eventsMatch)
            continue;
        if ((_a = preset.schedule) === null || _a === void 0 ? void 0 : _a.schedule) {
    ...
    if (acsReq)
        return sendAcsRequest(sessionContext, reqId, acsReq);
    session.clearProvisions(sessionContext);
    if (whiteList != null)
    ...
    for (const p of filteredPresets) {
        if (evaluate(p.precondition, {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e))) {
            const provs = p.provisions.map((pp) => [
                pp[0],
                ...pp
                    .slice(1)
                    .map((arg) => evaluate(arg, {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e))),
            ]);
...
async function endSession(sessionContext) {
    var _a;
    ...
    const promises = [];
    promises.push(db.saveDevice(sessionContext.deviceId, sessionContext.deviceData, sessionContext.new, sessionContext.timestamp));
    if (sessionContext.operationsTouched) {
        for (const k of Object.keys(sessionContext.operationsTouched)) {
            ...
            if (sessionContext.operations[k]) {
                promises.push(db.saveOperation(sessionContext.deviceId, k, sessionContext.operations[k]));
            }
            else {
    ...
    if ((_a = sessionContext.doneTasks) === null || _a === void 0 ? void 0 : _a.length) {
        saveCache = true;
        promises.push(db.clearTasks(sessionContext.deviceId, sessionContext.doneTasks));
    }
    if (sessionContext.faultsTouched) {
        for (const k of Object.keys(sessionContext.faultsTouched)) {
            ...
            if (sessionContext.faults[k]) {
                ...
                promises.push(db.saveFault(sessionContext.deviceId, k, sessionContext.faults[k]));
            }
            else {
    ...
    if (saveCache) {
        promises.push(cacheDueTasksAndFaultsAndOperations(sessionContext.deviceId, sessionContext.tasks, sessionContext.faults, sessionContext.operations, sessionContext.cacheUntil));
    }
    await Promise.all(promises);
    await cache.releaseLock(`cwmp_session_${sessionContext.deviceId}`, sessionContext.sessionId);
...
async function sendAcsRequest(sessionContext, id, acsRequest) {
    if (!acsRequest)
...
export function onConnection(socket) {
    socket.on("close", async () => {
        ...
        setTimeout(async () => {
            const sessionContextString = await cache.get(`session_${sessionContext.sessionId}`);
        ...
        await cache.set(`session_${sessionContext.sessionId}`, sessionContextString, Math.ceil(timeout / 1000) + 3);
    });
}
export function onClientError(err, socket) {
    const remoteAddress = getSocketEndpoints(socket).remoteAddress;
    localCache
        .getCurrentSnapshot()
        .then((cacheSnapshot) => {
        const debugEnabled = !!localCache.getConfig(cacheSnapshot, "cwmp.debug", {
        ...
        if (debugEnabled)
            debug.clientError(remoteAddress, err);
    })
        .catch((err) => {
        throw err;
    });
...
async function getDueTasksAndFaultsAndOperations(deviceId, timestamp) {
...
async function cacheDueTasksAndFaultsAndOperations(deviceId, tasks, faults, operations, cacheUntil) {
    const v = {
        tasks: null,
        faults: null,
        operations: null,
    ...
    await cache.set(`${deviceId}_tasks_faults_operations`, JSON.stringify(v), ttl);
}
async function reportBadState(sessionContext) {
    logger.accessError({
...
async function responseUnauthorized(sessionContext, close) {
    const resHeaders = {};
    if (close) {
        // Invalid credentials
        logger.accessError({
...
async function processRequest(sessionContext, rpc, parseWarnings, body) {
    var _a;
    ...
    if (sessionContext.state === 0) {
        if (((_a = rpc.cpeRequest) === null || _a === void 0 ? void 0 : _a.name) !== "Inform")
            return reportBadState(sessionContext);
        const res = await inform(sessionContext, rpc);
        sessionContext.debug = !!localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.debug", {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e));
        if (!sessionContext.timeout) {
            sessionContext.timeout = +localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.sessionTimeout", {}, sessionContext.timestamp, (e) => session.configContextCallback(sessionContext, e));
        }
...
async function clientError(httpRequest, httpResponse, sessionContext, body, msg) {
    ...
    if (sessionContext) {
        ...
        debugEnabled = !!localCache.getConfig(cacheSnapshot, "cwmp.debug", {
    ...
    if (sessionContext === null || sessionContext === void 0 ? void 0 : sessionContext.state)
        await endSession(sessionContext);
}
...
async function listenerAsync(httpRequest, httpResponse) {
    var _a, _b;
    ...
    if (sessionContext) {
        ...
        if ((sessionContext.sessionId !== sessionId && sessionContext.state) ||
    ...
    if (!sessionContext && sessionId && ((_a = rpc.cpeRequest) === null || _a === void 0 ? void 0 : _a.name) !== "Inform") {
        await new Promise((resolve) => setTimeout(resolve, 100));
        const sessionContextString = await cache.pop(`session_${sessionId}`);
        if (sessionContextString) {
            sessionContext = await session.deserialize(sessionContextString);
    ...
    const { tasks: dueTasks, faults, operations, ttl: cacheUntil, } = await getDueTasksAndFaultsAndOperations(deviceId, _sessionContext.timestamp);
    ...
    const parameters = await db.fetchDevice(_sessionContext.deviceId, _sessionContext.timestamp);
    if (parameters) {
