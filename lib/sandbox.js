...
function runExtension(sessionContext, key, extCall) {
    ...
    let prom = re.get(key);
    if (!prom) {
        re.set(key, (prom = new Promise((resolve, reject) => {
            extensions
                .run(extCall)
                .then(({ fault, value }) => {
                re.delete(key);
                if (!fault)
                    sessionContext.extensionsCache[key] = value;
                resolve(fault);
            })
                .catch(reject);
        })));
        ...
        for (const attrName of attributes) {
            Object.defineProperty(this, attrName, {
                get: function () {
                    ...
                    if (!unpacked.length)
                        return UNDEFINED;
                    const attr = state.sessionContext.deviceData.attributes.get(unpacked[0], state.revision)[attrName];
                    if (!attr)
                        return UNDEFINED;
                    return attr[1];
                },
    ...
    for (const [attrName, attrValue] of Object.entries(values)) {
        if (attrName === "path") {
            ...
            if (attrName === "value" && !Array.isArray(values.value))
                ...
                declaration.attrSet[attrName] = values[attrName];
        }
    }
...
function commit() {
    ...
    if (state.revision === state.maxRevision + 1) {
        for (const d of state.declarations)
            d.defer = false;
        throw COMMIT;
    }
...
function errorToFault(err) {
    ...
    if (err.stack) {
        ...
        const stackTrimIndex = fault.detail["stack"].match(/\s+at\s[^\s]+\s+at\s[^\s]+\s\(vm\.js.+\)/);
        if (stackTrimIndex) {
...
export async function run(script, globals, sessionContext, startRevision, maxRevision, extCounter = 0) {
    state = {
    ...
    for (const n of Object.keys(context))
        delete context[n];
    ...
    const _state = state;
    let fault;
    await Promise.all(Object.entries(_state.extensions).map(async ([k, v]) => {
    ...
    if (status === 2) {
        return run(script, globals, sessionContext, startRevision, maxRevision, extCounter - _state.extCounter);
    }
    return {
        fault: null,
        clear: _state.clear,
