...
export default class VersionedMap {
    ...
    set(key, value, rev = this._revision) {
        ...
        if (old === NONEXISTENT)
            ++this._sizeDiff[rev];
        for (let i = revisions.length; i < rev; ++i)
            revisions[i] = old;
        revisions[rev] = value;
        return this;
    }
    ...
    setRevisions(key, revisionsObj) {
        ...
        const mutations = Object.keys(revisionsObj).reduce((acc, cur) => (cur === "delete" ? acc : acc | (1 << +cur)), del);
        const revisions = [];
        let prev = NONEXISTENT;
    ...
    *diff() {
        for (const [key, revisions] of this.map) {
            ...
            let first = revisions[0];
            let last = revisions[revisions.length - 1];
            if (first === NONEXISTENT && last === NONEXISTENT)
                continue;
            if (first === NONEXISTENT)
                first = UNDEFINED;
            if (last === NONEXISTENT)
