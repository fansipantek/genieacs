...
export function init(deviceId, cwmpVersion, timeout) {
...
export function configContextCallback(sessionContext, exp) {
    var _a;
    ...
    if (exp[0] === "PARAM" && typeof exp[1] === "string") {
        ...
        else if (name === "productClass")
            name = "DeviceID.ProductClass";
        else if (name === "oui")
            name = "DeviceID.OUI";
        ...
        if (path) {
            ...
            if ((_a = attrs === null || attrs === void 0 ? void 0 : attrs.value) === null || _a === void 0 ? void 0 : _a[1])
                return attrs.value[1][0];
        }
    }
    else if (exp[0] === "FUNC") {
...
export async function inform(sessionContext, rpcReq) {
...
export async function transferComplete(sessionContext, rpcReq) {
    var _a;
    const revision = (sessionContext.revisions[sessionContext.revisions.length - 1] || 0) + 1;
    ...
    if (((_a = rpcReq.faultStruct) === null || _a === void 0 ? void 0 : _a.faultCode) !== "0") {
    ...
    const timestamp = sessionContext.timestamp + sessionContext.iteration + 1;
    let p;
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.LastDownload`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [operation.timestamp, "xsd:dateTime"]] }, toClear);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.LastFileType`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [operation.args.fileType, "xsd:string"]] }, toClear);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.LastFileName`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [operation.args.fileName, "xsd:string"]] }, toClear);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.LastTargetFileName`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [operation.args.targetFileName, "xsd:string"]] }, toClear);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.StartTime`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [+rpcReq.startTime, "xsd:dateTime"]] }, toClear);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.CompleteTime`));
    toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, [+rpcReq.completeTime, "xsd:dateTime"]] }, toClear);
    if (toClear) {
        for (const c of toClear)
            device.clear(sessionContext.deviceData, c[0], c[1], c[2], c[3]);
    }
...
function revertDownloadParameters(sessionContext, instance) {
    var _a;
    const timestamp = sessionContext.timestamp + sessionContext.iteration + 1;
    let p;
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.LastDownload`));
    const lastDownload = sessionContext.deviceData.attributes.get(p);
    p = sessionContext.deviceData.paths.add(Path.parse(`Downloads.${instance}.Download`));
    const toClear = device.set(sessionContext.deviceData, p, timestamp, {
        value: [timestamp, [((_a = lastDownload === null || lastDownload === void 0 ? void 0 : lastDownload.value[1]) === null || _a === void 0 ? void 0 : _a[0]) || 0, "xsd:dateTime"]],
    });
    if (toClear) {
        for (const c of toClear)
            device.clear(sessionContext.deviceData, c[0], c[1], c[2], c[3]);
...
export async function timeoutOperations(sessionContext) {
    const revision = (sessionContext.revisions[sessionContext.revisions.length - 1] || 0) + 1;
    sessionContext.deviceData.timestamps.revision = revision;
    sessionContext.deviceData.attributes.revision = revision;
    const faults = [];
    const operations = [];
    for (const [commandKey, operation] of Object.entries(sessionContext.operations)) {
        if (operation.name !== "Download")
            throw new Error(`Unknown operation name ${operation.name}`);
        const DOWNLOAD_TIMEOUT = +localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.downloadTimeout", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e)) * 1000;
        ...
        const SUCCESS_ON_TIMEOUT = +localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.downloadSuccessOnTimeout", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
...
export function addProvisions(sessionContext, channel, provisions) {
    ...
    const MAX_ITERATIONS = +localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.maxCommitIterations", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e)) * 2;
...
async function runProvisions(sessionContext, provisions, startRevision, endRevision) {
    const allProvisions = localCache.getProvisions(sessionContext.cacheSnapshot);
    const res = await Promise.all(provisions.map(async (provision) => {
        if (!allProvisions[provision[0]]) {
            if (defaultProvisions[provision[0]]) {
                ...
                let fault = null;
                try {
                    done = defaultProvisions[provision[0]](sessionContext, provision, dec, startRevision, endRevision);
                }
                catch (err) {
                    fault = {
        ...
        return sandbox.run(allProvisions[provision[0]].script, { args: provision.slice(1) }, sessionContext, startRevision, endRevision);
    }));
    let done = true;
    let allDeclarations = [];
    let allClear = [];
    ...
    for (const r of res) {
        ...
        if (r.declare)
            allDeclarations = allDeclarations.concat(r.declare);
        if (r.clear)
            allClear = allClear.concat(r.clear);
        fault = r.fault || fault;
    }
    if (done)
...
async function runVirtualParameters(sessionContext, provisions, startRevision, endRevision) {
    const allVirtualParameters = localCache.getVirtualParameters(sessionContext.cacheSnapshot);
    const res = await Promise.all(provisions.map(async (provision) => {
        const globals = {
            args: provision.slice(1),
        };
        const r = await sandbox.run(allVirtualParameters[provision[0]].script, globals, sessionContext, startRevision, endRevision);
        if (r.done && !r.fault) {
            if (!r.returnValue) {
                r.fault = {
                    code: "script",
            ...
            else if (provision[1].writable != null ||
            ...
            if (r.returnValue.value != null) {
                let v, t;
                if (Array.isArray(r.returnValue.value))
                    [v, t] = r.returnValue.value;
                else
                    v = r.returnValue.value;
                if (!t) {
                    if (typeof v === "number")
                        t = "xsd:int";
...
function runDeclarations(sessionContext, declarations) {
    ...
    for (const declaration of declarations) {
        ...
        if ((path.alias | path.wildcard) & 1 ||
            ...
            if ((path.alias | path.wildcard) & 2) {
                ...
                for (const k of Object.keys(allVirtualParameters)) {
                    sessionContext.deviceData.paths.add(Path.parse(`VirtualParameters.${k}`));
                }
            }
        }
        ...
        if (path.alias) {
            const aliasDecs = device.getAliasDeclarations(path, declaration.pathGet || 1);
            for (const ad of aliasDecs) {
                const p = sessionContext.deviceData.paths.add(ad.path);
                allDeclareTimestamps.set(p, Math.max(ad.pathGet || 1, allDeclareTimestamps.get(p) || 0));
                let attrTrackers;
                if (ad.attrGet) {
            ...
            for (const u of unpacked) {
                allDeclareTimestamps.set(u, Math.max(declaration.pathGet || 1, allDeclareTimestamps.get(u) || 0));
            ...
            allDeclareTimestamps.set(path, Math.max(declaration.pathGet || 1, allDeclareTimestamps.get(path) || 0));
            if (declaration.attrGet)
        ...
        if (declaration.pathSet != null) {
            ...
            if (!parent.wildcard && !parent.alias) {
                ...
                for (const u of unpacked)
                    mergeAttributeTimestamps(u, { writable: 1 });
                processInstances(sessionContext, parent, unpacked, keys, minInstances, maxInstances, declaration.defer);
            }
            else {
                const parentsUnpacked = device.unpack(sessionContext.deviceData, parent);
                for (const par of parentsUnpacked) {
                    const up = device.unpack(sessionContext.deviceData, par.concat(path.slice(-1)));
                    // Ensure writable attr is available
                    mergeAttributeTimestamps(par, { writable: 1 });
                    for (const u of up)
                        mergeAttributeTimestamps(u, { writable: 1 });
                    processInstances(sessionContext, par, up, keys, minInstances, maxInstances, declaration.defer);
                }
            }
        }
    }
...
export async function rpcRequest(sessionContext, _declarations) {
    var _a;
    ...
    if (!sessionContext.virtualParameters.length &&
        !sessionContext.declarations.length &&
        !(_declarations === null || _declarations === void 0 ? void 0 : _declarations.length) &&
        !sessionContext.provisions.length)
        return { fault: null, rpcId: null, rpc: null };
    if (sessionContext.declarations.length <=
        ...
        const { fault, clear: toClear, declare: decs, done: done, returnValue: ret, } = await run(sessionContext, provisions, sessionContext.revisions[inception - 1] || 0, sessionContext.revisions[inception]);
        if (fault) {
    ...
    if (_declarations === null || _declarations === void 0 ? void 0 : _declarations.length) {
        delete sessionContext.syncState;
        if (!sessionContext.declarations[0])
    ...
    if (!(sessionContext.syncState &&
        ...
        sessionContext.syncState.virtualParameterDeclarations.length >=
            sessionContext.declarations.length)) {
        const inception = sessionContext.syncState &&
            sessionContext.syncState.virtualParameterDeclarations
            ? sessionContext.syncState.virtualParameterDeclarations.length
            : 0;
        ...
        let vpd = runDeclarations(sessionContext, sessionContext.declarations[inception]);
        ...
        vpd = vpd.filter((declaration) => {
            if (Object.keys(allVirtualParameters).length) {
                if (declaration[0].length === 1) {
                    ...
                    if (declaration[0].segments[1] === "*") {
                        for (const k of Object.keys(allVirtualParameters)) {
                            toClear = device.set(sessionContext.deviceData, Path.parse(`VirtualParameters.${k}`), timestamp, {
                                object: [timestamp, 0],
                            }, toClear);
                        }
                    ...
                    else if (allVirtualParameters[declaration[0].segments[1]]) {
                        // Avoid setting on every inform as "exist" timestamp
                        // is not saved in DB
                        if (!sessionContext.deviceData.attributes.has(declaration[0])) {
            ...
            for (const p of sessionContext.deviceData.paths.find(declaration[0], false, true)) {
    ...
    let provisions = generateGetVirtualParameterProvisions(sessionContext, sessionContext.syncState.virtualParameterDeclarations[inception]);
    if (!provisions) {
        ...
        if (!sessionContext.rpcRequest) {
            ...
            for (const instance of sessionContext.syncState.downloadsToCreate) {
                if (index == null) {
                    index = 0;
                    for (const p of sessionContext.deviceData.paths.find(Path.parse("Downloads.*"), false, true)) {
                        if (+p.segments[1] > index &&
                            sessionContext.deviceData.attributes.has(p))
                            index = +p.segments[1];
                    }
                }
                ++index;
                toClear = device.set(sessionContext.deviceData, Path.parse("Downloads"), timestamp, { object: [timestamp, 1], writable: [timestamp, 1] }, toClear);
                toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${index}`), timestamp, { object: [timestamp, 1], writable: [timestamp, 1] }, toClear);
                const params = {
                    FileType: {
                        writable: 1,
                ...
                for (const [k, v] of Object.entries(params)) {
                    toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${index}.${k}`), timestamp, {
                        object: [timestamp, 0],
                ...
                toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${index}.*`), timestamp, null, toClear);
            }
            ...
            for (const [p, v] of sessionContext.syncState.downloadsValues) {
                ...
                if (attrs) {
                    if (((_a = attrs.writable) === null || _a === void 0 ? void 0 : _a[1]) && attrs.value) {
                        const val = device.sanitizeParameterValue([v, attrs.value[1][1]]);
                        if (val[0] !== attrs.value[1][0]) {
                            toClear = device.set(sessionContext.deviceData, p, timestamp, { value: [timestamp, val] }, toClear);
                        }
                    }
                }
            }
    ...
    const rev = sessionContext.revisions[sessionContext.revisions.length - 1] || 0;
    ...
    for (const [i, vpu] of ret.entries()) {
        ...
        toClear = device.set(sessionContext.deviceData, Path.parse(`VirtualParameters.${vparams[i][0]}`), timestamp, vpu, toClear);
    }
    if (toClear) {
        for (const c of toClear)
            device.clear(sessionContext.deviceData, c[0], c[1], c[2], c[3]);
...
function generateGetRpcRequest(sessionContext) {
    ...
    for (const path of syncState.refreshAttributes.exist) {
        let found = false;
        for (const p of sessionContext.deviceData.paths.find(path, false, true, 99)) {
            if (syncState.refreshAttributes.value.has(p) ||
    ...
    if (syncState.gpn.size) {
        const GPN_NEXT_LEVEL = localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.gpnNextLevel", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
        const paths = Array.from(syncState.gpn.keys()).sort((a, b) => b.length - a.length);
        let path = paths.pop();
        // Skip root GPN workaround
        if (path && !path.length) {
            const SKIP_ROOT_GPN = !!localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.skipRootGpn", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
            if (SKIP_ROOT_GPN)
                path = paths.pop();
        }
        while (path &&
        ...
        if (path) {
            ...
            if (path.length >= GPN_NEXT_LEVEL) {
                ...
                for (const p of sessionContext.deviceData.paths.find(path, true, false, 99)) {
    ...
    if (syncState.refreshAttributes.value.size) {
        const GPV_BATCH_SIZE = localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.gpvBatchSize", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
        ...
        for (const path of syncState.refreshAttributes.value) {
            ...
            if (((_a = attrs === null || attrs === void 0 ? void 0 : attrs.object) === null || _a === void 0 ? void 0 : _a[1]) === 0) {
    ...
    if (syncState.refreshAttributes.notification.size ||
...
function compareAccessLists(list1, list2) {
    if (list1.length !== list2.length)
        return false;
    for (const [i, v] of list1.entries())
        if (v !== list2[i])
            return false;
    return true;
}
function generateSetRpcRequest(sessionContext) {
    ...
    if (!syncState)
        return null;
    const deviceData = sessionContext.deviceData;
    const SKIP_WRITABLE_CHECK = !!localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.skipWritableCheck", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
    const canWrite = (attrs) => SKIP_WRITABLE_CHECK || (attrs.writable && !!attrs.writable[1]);
    // Delete instance
    ...
    // Set values
    const GPV_BATCH_SIZE = localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.gpvBatchSize", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
    const DATETIME_MILLISECONDS = !!localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.datetimeMilliseconds", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
    const BOOLEAN_LITERAL = !!localCache.getConfig(sessionContext.cacheSnapshot, "cwmp.booleanLiteral", {}, sessionContext.timestamp, (e) => configContextCallback(sessionContext, e));
    const parameterValues = [];
    for (const [k, v] of syncState.spv) {
        syncState.spv.delete(k);
        const attrs = sessionContext.deviceData.attributes.get(k);
        const curVal = (_a = attrs.value) === null || _a === void 0 ? void 0 : _a[1];
    ...
    for (const [k, v] of syncState.spa) {
        ...
        if (parameterAttributes.length >= GPV_BATCH_SIZE)
            break;
    }
    ...
    for (const [p, t] of syncState.downloadsDownload) {
        if (!(t > 0 && t <= sessionContext.timestamp))
            continue;
        const attrs = deviceData.attributes.get(p);
        if (!(t <= ((_c = (_b = attrs === null || attrs === void 0 ? void 0 : attrs.value) === null || _b === void 0 ? void 0 : _b[1]) === null || _c === void 0 ? void 0 : _c[0]))) {
            const fileTypeAttrs = deviceData.attributes.get(deviceData.paths.get(p.slice(0, -1).concat(Path.parse("FileType"))));
            const fileNameAttrs = deviceData.attributes.get(deviceData.paths.get(p.slice(0, -1).concat(Path.parse("FileName"))));
            const targetFileNameAttrs = deviceData.attributes.get(deviceData.paths.get(p.slice(0, -1).concat(Path.parse("TargetFileName"))));
            return {
                name: "Download",
                commandKey: generateRpcId(sessionContext),
                instance: p.segments[1],
                fileType: (_d = fileTypeAttrs === null || fileTypeAttrs === void 0 ? void 0 : fileTypeAttrs.value) === null || _d === void 0 ? void 0 : _d[1][0],
                fileName: (_e = fileNameAttrs === null || fileNameAttrs === void 0 ? void 0 : fileNameAttrs.value) === null || _e === void 0 ? void 0 : _e[1][0],
                targetFileName: (_f = targetFileNameAttrs === null || targetFileNameAttrs === void 0 ? void 0 : targetFileNameAttrs.value) === null || _f === void 0 ? void 0 : _f[1][0],
            };
        }
    }
    // Reboot
    if (syncState.reboot > 0 && syncState.reboot <= sessionContext.timestamp) {
        ...
        if (!(((_g = attrs === null || attrs === void 0 ? void 0 : attrs.value) === null || _g === void 0 ? void 0 : _g[1][0]) >= syncState.reboot)) {
            delete syncState.reboot;
            return { name: "Reboot" };
        }
    }
    // Factory reset
    if (syncState.factoryReset > 0 &&
        syncState.factoryReset <= sessionContext.timestamp) {
        const p = sessionContext.deviceData.paths.get(Path.parse("FactoryReset"));
        const attrs = p ? sessionContext.deviceData.attributes.get(p) : null;
        if (!(((_h = attrs === null || attrs === void 0 ? void 0 : attrs.value) === null || _h === void 0 ? void 0 : _h[1][0]) >= syncState.factoryReset)) {
...
function generateGetVirtualParameterProvisions(sessionContext, virtualParameterDeclarations) {
    let provisions;
...
function generateSetVirtualParameterProvisions(sessionContext, virtualParameterDeclarations) {
    var _a;
    let provisions;
    if (virtualParameterDeclarations) {
        for (const declaration of virtualParameterDeclarations) {
            if (((_a = declaration[2]) === null || _a === void 0 ? void 0 : _a.value) != null) {
                const attrs = sessionContext.deviceData.attributes.get(declaration[0]);
                if (attrs &&
                    attrs.writable &&
                    attrs.writable[1] &&
...
function processDeclarations(sessionContext, allDeclareTimestamps, allDeclareAttributeTimestamps, allDeclareAttributeValues) {
    const deviceData = sessionContext.deviceData;
    const syncState = sessionContext.syncState;
    const root = sessionContext.deviceData.paths.add(Path.parse(""));
    const paths = deviceData.paths.find(root, false, true, 99);
    paths.sort((a, b) => a.wildcard === b.wildcard ? a.length - b.length : a.wildcard - b.wildcard);
    ...
    function func(leafParam, leafIsObject, leafTimestamp, _paths) {
        ...
        for (const path of _paths) {
            if (path.length > currentPath.length) {
                const fragment = path.segments[currentPath.length];
        ...
        if (currentAttributes) {
            ...
            leafIsObject = (_a = currentAttributes.object) === null || _a === void 0 ? void 0 : _a[1];
        ...
        switch (currentPath.segments[0] !== "*"
            ? currentPath.segments[0]
            : leafParam.segments[0]) {
            case "Reboot":
                if (currentPath.length === 1) {
                    if (declareAttributeValues === null || declareAttributeValues === void 0 ? void 0 : declareAttributeValues.value)
            ...
            case "VirtualParameters":
                if (currentPath.length <= 2) {
                    ...
                    if (currentPath.wildcard === 0) {
                        if (declareAttributeTimestamps) {
                            for (const [attrName, attrTimestamp] of Object.entries(declareAttributeTimestamps)) {
                                if (!(currentAttributes &&
            ...
            default:
                if (declareTimestamp > currentTimestamp &&
                    ...
                    if (currentPath === leafParam) {
                        ...
                        if (leafTimestamp > 0) {
                            ...
                            const f = ((1 << currentPath.length) - 1) ^ ((1 << leafParam.length) - 1);
                        ...
                        if (leafIsObject == null) {
                            const f = ((1 << syncState.gpnPatterns.size) - 1) ^
                ...
                if (currentAttributes) {
                    if (declareAttributeTimestamps) {
                        for (const [attrName, attrTimestamp] of Object.entries(declareAttributeTimestamps)) {
                            if (!(currentAttributes[attrName] &&
                                attrTimestamp <= currentAttributes[attrName][0])) {
                                if (attrName === "value") {
                                    if (!(currentAttributes.object &&
        ...
        for (let [fragment, child] of children) {
            ...
            if (((currentPath.wildcard ^ child[0].wildcard) &
                ...
                if (fragment !== "*") {
                    ...
                    if (wildcardChild)
                        child = child.concat(wildcardChild);
                }
                func(leafParam, leafIsObject, leafTimestamp, child);
            }
        }
    }
    if (allDeclareTimestamps.size ||
        allDeclareAttributeTimestamps.size ||
        allDeclareAttributeValues.size)
        func(root, 1, 0, paths);
    return virtualParameterDeclarations;
}
function processInstances(sessionContext, parent, parameters, keys, minInstances, maxInstances, defer) {
    parent = sessionContext.deviceData.paths.add(parent);
...
export async function rpcResponse(sessionContext, id, _rpcRes) {
    ...
    if (rpcRes.name === "GetParameterValuesResponse") {
        ...
        for (const [path, value, type] of rpcRes.parameterList) {
            ...
            toClear = device.set(sessionContext.deviceData, path, timestamp, {
                object: [timestamp, 0],
        ...
        if (requested.size) {
            for (const p of requested) {
                ...
                toClear = device.set(sessionContext.deviceData, Path.parse(p), timestamp, {
                    object: [timestamp, 0],
        ...
        if (rpcReq.parameterPath.endsWith("."))
            ...
            root = Path.parse(rpcReq.parameterPath);
        ...
        rpcRes.parameterList.sort((a, b) => {
            ...
            for (let i = 0; i < l; ++i) {
                if (pa.segments[i] > pb.segments[i])
                    return 1;
                if (pa.segments[i] < pb.segments[i])
                    return -1;
            }
        ...
        for (let idx = 1; idx < rpcRes.parameterList.length; ++idx) {
            ...
            for (let i = cur.length - 2; i >= 0; --i) {
                if (i < prev.length && prev.segments[i] === cur.segments[i]) {
                    ...
                    if (i === prev.length - 1)
                        rpcRes.parameterList[idx - 1][1] = true;
                    break;
                }
        ...
        const wildcardParams = [root.concat(wildcardPath)];
        ...
        for (const path of wildcardParams) {
            toClear = device.set(sessionContext.deviceData, path, timestamp, null, toClear);
        }
    }
        ...
        if (rpcReq.name !== "SetParameterValues")
            return invalidResponse("Response name does not match request name");
        for (const p of rpcReq.parameterList) {
            toClear = device.set(sessionContext.deviceData, Path.parse(p[0]), timestamp + 1, {
        ...
        for (const p of rpcReq.parameterList) {
            ...
            toClear = device.set(sessionContext.deviceData, Path.parse(p[0]), timestamp + 1, attrs, toClear);
        }
    }
    else if (rpcRes.name === "AddObjectResponse") {
        if (rpcReq.name !== "AddObject")
            return invalidResponse("Response name does not match request name");
        toClear = device.set(sessionContext.deviceData, Path.parse(rpcReq.objectName + rpcRes.instanceNumber), timestamp + 1, { object: [timestamp + 1, 1] }, toClear);
    }
    else if (rpcRes.name === "DeleteObjectResponse") {
        if (rpcReq.name !== "DeleteObject")
            return invalidResponse("Response name does not match request name");
        toClear = device.set(sessionContext.deviceData, Path.parse(rpcReq.objectName.slice(0, -1)), timestamp + 1, null, toClear);
    }
    else if (rpcRes.name === "RebootResponse") {
        if (rpcReq.name !== "Reboot")
            return invalidResponse("Response name does not match request name");
        toClear = device.set(sessionContext.deviceData, Path.parse("Reboot"), timestamp + 1, { value: [timestamp + 1, [sessionContext.timestamp, "xsd:dateTime"]] }, toClear);
    }
    else if (rpcRes.name === "FactoryResetResponse") {
        if (rpcReq.name !== "FactoryReset")
            return invalidResponse("Response name does not match request name");
        toClear = device.set(sessionContext.deviceData, Path.parse("FactoryReset"), timestamp + 1, { value: [timestamp + 1, [sessionContext.timestamp, "xsd:dateTime"]] }, toClear);
    }
    else if (rpcRes.name === "DownloadResponse") {
        if (rpcReq.name !== "Download")
            return invalidResponse("Response name does not match request name");
        toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.Download`), timestamp + 1, { value: [timestamp + 1, [sessionContext.timestamp, "xsd:dateTime"]] }, toClear);
        if (rpcRes.status === 0) {
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.LastDownload`), timestamp + 1, {
                value: [timestamp + 1, [sessionContext.timestamp, "xsd:dateTime"]],
            }, toClear);
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.LastFileType`), timestamp + 1, { value: [timestamp + 1, [rpcReq.fileType, "xsd:string"]] }, toClear);
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.LastFileName`), timestamp + 1, { value: [timestamp + 1, [rpcReq.fileType, "xsd:string"]] }, toClear);
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.LastTargetFileName`), timestamp + 1, { value: [timestamp + 1, [rpcReq.fileType, "xsd:string"]] }, toClear);
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.StartTime`), timestamp + 1, { value: [timestamp + 1, [+rpcRes.startTime, "xsd:dateTime"]] }, toClear);
            toClear = device.set(sessionContext.deviceData, Path.parse(`Downloads.${rpcReq.instance}.CompleteTime`), timestamp + 1, { value: [timestamp + 1, [+rpcRes.completeTime, "xsd:dateTime"]] }, toClear);
        }
        else {
            const operation = {
                name: "Download",
...
export async function rpcFault(sessionContext, id, faultResponse) {
    const rpcReq = sessionContext.rpcRequest;
    delete sessionContext.syncState;
    delete sessionContext.rpcRequest;
    ++sessionContext.rpcCount;
    ...
    if (faultResponse.detail.faultCode === "9005") {
        ...
        if (rpcReq.name === "GetParameterNames") {
            ...
            toClear = rpcReq.parameterNames.map((p) => [Path.parse(p.replace(/\.$/, "")), timestamp]);
        }
        else if (rpcReq.name === "SetParameterValues") {
            toClear = rpcReq.parameterList.map((p) => [Path.parse(p[0].replace(/\.$/, "")), timestamp]);
        }
...
export async function serialize(sessionContext) {
    const deviceData = [];
    for (const path of sessionContext.deviceData.paths.find(Path.parse(""), false, false, 99)) {
        const e = [
            path.toString(),
