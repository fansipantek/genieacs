...
function isPrintable(str) {
    return !/[^\t\n\x20-\x7e\x85\u{a0}-\u{d7ff}\u{e000}-\u{fffd}\u{10000}-\u{10ffff}]/u.test(str);
}
...
function stringifyString(str, res, prefix1, prefix2) {
    ...
    if (lines.length > 1) {
        ...
        if (!isFolded) {
            res.push(`${prefix1}|${idt}${chmp}`, ...lines.map((l) => (l ? prefix2 + l : l)));
            return;
        }
        ...
        for (let i = 1; i < folded.length; ++i) {
            ...
            if (prevLine && !folded[i - 1][0].startsWith(" "))
                res.push("");
...
function stringifyAny(obj, res, prefix1 = "", prefix2 = "") {
    ...
    if (!prefix1 || prefix1.endsWith("- ")) {
        stringifyAny(entries[0][1], res, prefix1 + `${stringifyKey(entries[0][0])}: `, prefix2 + INDENTATION);
        prefix1 = prefix2;
        prefix2 = prefix2 + INDENTATION;
        for (let i = 1; i < entries.length; ++i) {
            stringifyAny(entries[i][1], res, prefix1 + `${stringifyKey(entries[i][0])}: `, prefix2);
        }
    }
    else {
        res.push(prefix1);
