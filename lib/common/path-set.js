...
export default class PathSet {
    ...
    add(path) {
        if (path.alias)
            throw new Error("PathSet does not support aliased paths");
    ...
    find(path, superset = false, subset = false, depth = path.length) {
        if (path.alias)
            throw new Error("PathSet does not support aliased paths");
        ...
        for (let i = len - 1; i >= 0; --i) {
            ...
            if (!fragmentIndexSet1) {
                if (!fragmentIndexSet2)
                    return [];
                if (!res)
                    res = [...fragmentIndexSet2];
                else
                    res = res.filter((r) => fragmentIndexSet2.has(r));
            }
            else if (!fragmentIndexSet2) {
                if (!res)
                    res = [...fragmentIndexSet1];
                else
                    res = res.filter((r) => fragmentIndexSet1.has(r));
            }
            else {
                if (!res) {
                    ...
                    res = res.filter((r) => fragmentIndexSet1.has(r) || fragmentIndexSet2.has(r));
                }
            }
            if (!res.length)
                return res;
        ...
        if (!res)
            res = [].concat(...lengthIndex.map((a) => [...a]));
        else
            res = res.filter((r) => lengthIndex.some((a) => a.has(r)));
        return res;
    }
}
