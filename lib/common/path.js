...
export default class Path {
    static parseAliasValue(pattern, index) {
        let i = index;
        while (charCodeAt(pattern, i) === CHAR_SPACE)
            ++i;
        if (charCodeAt(pattern, i) === CHAR_QUOTE) {
            for (let j = i + 1; j < pattern.length; ++j) {
                if (pattern.charCodeAt(j) === CHAR_QUOTE &&
        ...
        for (; i < pattern.length; ++i) {
            ...
            if (c === CHAR_CLOSE_BRACKET || c === CHAR_COMMA)
                break;
        }
        return { index: i, value: pattern.slice(index, i).trim() };
    }
    static parseAliasPath(pattern, index) {
        let i = index;
        while (charCodeAt(pattern, i) === CHAR_SPACE)
            ++i;
        ...
        while (charCodeAt(pattern, i) === CHAR_SPACE)
            ++i;
        if (charCodeAt(pattern, i) !== CHAR_COLON)
            return { index, path: null };
        return { index: i + 1, path: new Path(segments) };
    }
    static parseAlias(pattern, index) {
        const alias = [];
        let i = index;
        for (;;) {
            let path;
            ...
            index = i;
            if (charCodeAt(pattern, index) !== CHAR_COMMA)
                break;
            ++i;
        }
    ...
    static parsePath(pattern, index) {
        const segments = [];
        let segStart = index;
        let wildcard = -1;
        for (let i = index; i <= pattern.length; ++i) {
            ...
            if (c === CHAR_OPEN_BRACKET && i === segStart) {
                ...
                if (charCodeAt(pattern, idx) !== CHAR_CLOSE_BRACKET)
                    break;
                segments.push(alias);
                index = i = idx + 1;
                segStart = i + 1;
    ...
    constructor(segments) {
        ...
        const arr = segments.map((s, i) => {
            if (Array.isArray(s)) {
                alias |= 1 << i;
                const parts = s.map((al) => `${al[0].toString()}:${JSON.stringify(al[1])}`);
                return `[${parts.join(",")}]`;
            }
            else if (s === "*") {
                wildcard |= 1 << i;
    ...
    static parse(str) {
        ...
        if (!path) {
            ...
            if (!path) {
                ...
                if (path.toString() !== str)
                    cache1.set(path.toString(), path);
            }
            cache1.set(str, path);
        }
        return path;
    ...
    slice(start = 0, end = this.segments.length) {
        if (start < 0)
            start = Math.max(0, this.segments.length + start);
        if (end < 0)
            end = Math.max(0, this.segments.length + end);
        let str;
        if (start >= end) {
            str = "";
        }
            ...
            const i2 = end <= this.segments.length
