...
import { getCurrentSnapshot, getConfig, getConfigExpression, } from "./local-cache";
import { httpConnectionRequest, udpConnectionRequest, xmppConnectionRequest, } from "./connection-request";
...
export async function connectionRequest(deviceId, device) {
    if (!device) {
    ...
    const evalCallback = (exp) => {
        ...
        if (exp[0] === "PARAM" && typeof exp[1] === "string") {
            ...
            if (exp[1] === "REMOTE_ADDRESS")
                return remoteAddress;
            else if (exp[1] === "USERNAME")
                return username;
            else if (exp[1] === "PASSWORD")
                return password;
        }
        return exp;
    };
    ...
    const now = Date.now();
    const UDP_CONNECTION_REQUEST_PORT = +getConfig(snapshot, "cwmp.udpConnectionRequestPort", {}, now, evalCallback);
    const CONNECTION_REQUEST_TIMEOUT = +getConfig(snapshot, "cwmp.connectionRequestTimeout", {}, now, evalCallback);
    const CONNECTION_REQUEST_ALLOW_BASIC_AUTH = !!getConfig(snapshot, "cwmp.connectionRequestAllowBasicAuth", {}, now, evalCallback);
    let authExp = getConfigExpression(snapshot, "cwmp.connectionRequestAuth");
    ...
    if (udpConnectionRequestAddress && +stunEnable) {
        udpProm = udpConnectionRequest(udpConnectionRequestAddress, authExp, UDP_CONNECTION_REQUEST_PORT, debug, deviceId);
    }
    let status;
    if (connReqJabberId && XMPP_CONFIGURED) {
        status = await xmppConnectionRequest(connReqJabberId, authExp, CONNECTION_REQUEST_TIMEOUT, debug, deviceId);
    }
    else {
        status = await httpConnectionRequest(connectionRequestUrl, authExp, CONNECTION_REQUEST_ALLOW_BASIC_AUTH, CONNECTION_REQUEST_TIMEOUT, debug, deviceId);
        if (udpProm) {
            await udpProm;
            return "";
        }
...
export async function awaitSessionStart(deviceId, lastInform, timeout) {
    const now = Date.now();
    const device = await db.devicesCollection.findOne({ _id: deviceId }, { projection: { _lastInform: 1 } });
...
export async function awaitSessionEnd(deviceId, timeout) {
    const now = Date.now();
    const token = await cache.get(`cwmp_session_${deviceId}`);
    if (!token)
        return true;
...
function sanitizeTask(task) {
    ...
    switch (task.name) {
        ...
        case "download":
            ...
            if (task.targetFileName != null &&
                typeof task.targetFileName !== "string")
                throw new Error("Invalid 'targetFileName' property");
            break;
        case "provisions":
            if (!Array.isArray(task.provisions) ||
                !task.provisions.every((arr) => arr.every((s) => s == null || ["boolean", "number", "string"].includes(typeof s))))
...
export async function insertTasks(tasks) {
    if (tasks && !Array.isArray(tasks))
        tasks = [tasks];
    else if (!(tasks === null || tasks === void 0 ? void 0 : tasks.length))
        return tasks || [];
