...
function computeHash(snapshot) {
    ...
    for (const k of keys)
        h.update(snapshot.provisions[k].md5);
    keys = Object.keys(snapshot.virtualParameters).sort();
    h.update(JSON.stringify(keys));
    for (const k of keys)
        h.update(snapshot.virtualParameters[k].md5);
    keys = Object.keys(snapshot.config).sort();
    h.update(JSON.stringify(keys));
    for (const k of keys)
        h.update(JSON.stringify(snapshot.config[k]));
    keys = Object.keys(snapshot.files).sort();
    h.update(JSON.stringify(keys));
    for (const k of keys)
        h.update(JSON.stringify(snapshot.files[k]));
    keys = Object.keys(snapshot.users).sort();
    h.update(JSON.stringify(keys));
    for (const k of keys)
        h.update(JSON.stringify(snapshot.users[k]));
...
async function fetchPresets() {
    ...
    objects = objects.map((obj) => {
        ...
        if (!((_a = obj["_keys"]) === null || _a === void 0 ? void 0 : _a.length))
    ...
    for (const preset of res) {
        ...
        if (preset["precondition"]) {
            try {
                ...
                precondition = convertOldPrecondition(JSON.parse(preset["precondition"]));
            }
            // Simplify expression
...
async function fetchVirtualParameters() {
...
export function getPresets(snapshotKey) {
    ...
    if (!snapshot)
        throw new Error("Cache snapshot does not exist");
...
export function getConfig(snapshotKey, key, context, now, cb) {
    const snapshot = snapshots.get(snapshotKey);
    if (!snapshot)
        throw new Error("Cache snapshot does not exist");
    const oldOpts = {
...
export function getConfigExpression(snapshotKey, key) {
    const snapshot = snapshots.get(snapshotKey);
    if (!snapshot)
        throw new Error("Cache snapshot does not exist");
