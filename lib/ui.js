...
const getAuthorizer = memoize((snapshot, rolesStr) => {
...
koa.use(async (ctx, next) => {
    var _a;
    let roles = [];
    if ((_a = ctx.state.user) === null || _a === void 0 ? void 0 : _a.username) {
        let user;
        if (ctx.state.user.authMethod === "local") {
            user = localCache.getUsers(ctx.state.configSnapshot)[ctx.state.user.username];
        }
        else {
    ...
    ctx.state.authorizer = getAuthorizer(ctx.state.configSnapshot, JSON.stringify(roles));
    return next();
});
...
router.get("/init", async (ctx) => {
    ...
    if (Object.keys(localCache.getUsers(ctx.state.configSnapshot)).length) {
        if (!ctx.state.authorizer.hasAccess("users", 3))
            status["users"] = false;
  ...
  <html>
    ...
    <noscript>GenieACS UI requires JavaScript to work. Please enable JavaScript in your browser.</noscript>
      <script>
        ...
        window.username = ${JSON.stringify(ctx.state.user ? ctx.state.user.username : "")};
